...
## Bastion Host Verifications
  ...
  aws configure
      provide the required Access key ID,Secret access key

  aws eks update-kubeconfig --region us-east-1 --name roboshop-dev
  kubectl get nodes


## SonarQube Installation And Login
  ...
  The default credentials for a fresh SonarQube Community Edition instance (including when run via your Docker command) 
  are:
    Username: admin
    Password: admin
  Access the web interface at http://<your-server-ip>:9000  And change the password 
